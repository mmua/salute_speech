...
class TestAudioValidator(unittest.TestCase):
    ...
    def test_valid_encodings(self):
        ...
        self.assertEqual(
            AudioValidator.VALID_ENCODINGS["PCM_S16LE"]["sample_rate_range"],
            (8000, 96000),
        )
    ...
    def test_validate_params_valid_pcm(self):
        ...
        encoding, sample_rate, channels = AudioValidator._validate_params(
            "PCM_S16LE", 16000, 8
        )
    ...
    @patch("salute_speech.utils.audio.mediainfo")
    @patch("tempfile.NamedTemporaryFile")
    def test_detect_params_cleanup_error(
        self, mock_temp_file, mock_mediainfo, mock_unlink
    ):
